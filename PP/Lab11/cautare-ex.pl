...
% călători cu barca.
boat(NM, NC) :- member(NM,[0,1,2]), member(NC,[0,1,2]), NM + NC =< 2, NM + NC >= 1.
...
% problemă.
safeMisionari(NM, NC) :- (NM >= NC, NM\= 0; NM = 0).
...
% dată.
parseState(state(Mal, NM_Est, NC_Est, NM_Vest, NC_Vest), Mal, NM_Est, NC_Est, NM_Vest, NC_Vest).
...
% S1.
next_state(misionari, state(est, ME1, CE1, MV1, CV1), state(vest, ME2, CE2, MV2, CV2)) :- 
    boat(M, C), M =< ME1, C =< CE1,
    ME2 is ME1 - M, CE2 is CE1 - C, 
    MV2 is MV1 + M, CV2 is CV1 + C,  
    safeMisionari(ME2, CE2), safeMisionari(MV2, CV2).
next_state(misionari, state(vest, ME1, CE1, MV1, CV1), state(est, ME2, CE2, MV2, CV2)) :-
    boat(M, C), M =< MV1, C =< CV1,
    ME2 is ME1 + M, CE2 is CE1 + C, 
    MV2 is MV1 - M, CV2 is CV1 - C,  
    safeMisionari(ME2, CE2), safeMisionari(MV2, CV2).
...
preorder(N, Parc) :- preorderHelp([N], Parc).

preorderHelp([],[]).
preorderHelp([H|T], [H|Parc]) :- nod(H), arc(H, Children), append(Children, T, L), preorderHelp(L, Parc).
preorderHelp([H|T], [H|Parc]) :- nod(H), \+ arc(H, _), preorderHelp(T, Parc).
...
% unui arbore.
isRoot(Nod) :- nod(Nod), findall(N, (arc(N, Children), member(Nod, Children)), Bag), length(Bag, 0).
trees(TT) :- findall(List, (isRoot(Nod), preorder(Nod, List)), TT).

...
lengthV (V x y z) = sqrt $ (x * x) + (y * y) + (z * z)

normalizeV :: Vector -> Vector
normalizeV (V x y z) = V (x / len) (y / len) (z / len)
  where len = lengthV (V x y z)

dotV :: Vector -> Vector -> Double
dotV (V x1 y1 z1) (V x2 y2 z2) = (x1 * x2) + (y1 * y2) + (z1 * z2)

crossV :: Vector -> Vector -> Vector
crossV (V x1 y1 z1) (V x2 y2 z2) = V (y1 * z2 - z1 * y2) (z1 * x2 - x1 * z2) (x1 * y2 - y1 * x2)

addV :: Vector -> Vector -> Vector
addV (V x1 y1 z1) (V x2 y2 z2) = V (x1 + x2) (y1 + y2) (z1 + z2)

subV :: Vector -> Vector -> Vector
subV (V x1 y1 z1) (V x2 y2 z2) = V (x1 - x2) (y1 - y2) (z1 - z2)

orthogonalV :: [Vector] -> Bool
orthogonalV [] = True
orthogonalV [x] = True 
orthogonalV (x:y:xs) = if dotV x y == 0 then orthogonalV xs else False
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = ConsList (SList a) (SList a) | Cons a (SList a) | Nil deriving Show

emptySList :: SList a
emptySList = Nil

consElem :: a -> SList a -> SList a
consElem x l = Cons x l

consList :: SList a -> SList a -> SList a
consList l1 l2 = ConsList l1 l2

headSList :: SList a -> SList a
headSList Nil = Nil
headSList (Cons a l) = Cons a Nil
headSList (ConsList l1 l2) = l1

tailSList :: SList a -> SList a
tailSList Nil = Nil
tailSList (Cons a l) = l
tailSList (ConsList l1 l2) = l2

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual Nil Nil = True
deepEqual (Cons x l1) (Cons y l2) = if x == y then deepEqual l1 l2 else False
deepEqual (ConsList l1 l2) (ConsList l3 l4) = if deepEqual l1 l3 then deepEqual l2 l4 else False 


flatten :: SList a -> [a]
flatten Nil = []
flatten (Cons a l) = a : flatten l
flatten (ConsList l1 l2) = flatten l1 ++ flatten l2
...
-}
data BST a = NodeBST a (BST a) (BST a) | BSTNil deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil elem = NodeBST elem BSTNil BSTNil
insertElem (NodeBST x left right) elem = if elem <= x then (NodeBST x (insertElem left elem) right) else (NodeBST x left (insertElem right elem))

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil elem = Nothing
findElem (NodeBST x BSTNil BSTNil) elem = if x == elem then Just elem else Nothing
findElem (NodeBST x left right) elem
  | elem < x = findElem left elem
  | elem > x = findElem right elem
  | otherwise = Just elem

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (NodeBST x left right) elem1 elem2
  | (elem1 < x) && (elem2 < x) = subTree left elem1 elem2
  | (findElem left elem2) == Nothing && (findElem right elem2) == Nothing = Nothing
  | otherwise = Just (NodeBST x left right)

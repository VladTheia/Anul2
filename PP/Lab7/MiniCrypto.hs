...
arithmetic x y = x : (arithmetic (x + y) y) 
geometric :: Num a => a -> a -> [a]
geometric x y = x : (geometric (x * y) y) 
{-
  3. (2p)
...
--(fst (randomR (0, 255) (mkStdGen seed))) : randomR (0, 255) (snd (randomR (0, 255) (mkStdGen seed)))                   
{-
    4. (1p)
...
wordToAlpha x = let r = (fromIntegral x)   `mod` 26 in
  (chr (r + 97))
{-
    5. (1p)
...
tableToFunc (x:xs) char = if (char == fst x) then snd x else tableToFunc xs char

...
primes :: Int -> [Int]
primes n = [x | x <- [2,3..n], (length (divisors x)) == 2]
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc a =  map (\x -> (head x, length x)) (group2 (sort a))
...
-}
dup :: String -> String
dup l =  unwords (map (\x -> x ++ " " ++ x) (words l)) 
...
isIsogram x 
  | x == [] = True
  | not (isLetter (head x)) = isIsogram (tail x)
  | (isLetter (head x)) && not (elem (head x) (tail x)) = isIsogram (tail x)
  | otherwise = False

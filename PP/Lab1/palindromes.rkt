...
(define basis '(2 3 4 5 6 7 8 9 10))

(define (howManyBasisPal? n Basis x)
  (if (null? Basis)
      x
      (if (all-palindromes? n (list (car Basis)))
          (howManyBasisPal? n (cdr Basis) (+ x 1))
          (howManyBasisPal? n (cdr Basis) x))))

(define (first-b-pal start b)
  (if (= (howManyBasisPal? start basis 0) b)
      start

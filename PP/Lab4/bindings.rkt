...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([seg (get-line-segment)]
           [start (get-start-point seg)]
           [end (get-end-point seg)])
    (sqrt (+ (* (- (car start) (car end)) (- (car start) (car end)))
             (* (- (cdr start) (cdr end)) (- (cdr start) (cdr end)))))
    ))
...
(define (distance p1 p2)
  (compute-length (lambda () (cons p1 p2)) get-start-point get-end-point))
...
(define (compute-f-with-step f a b step)
  (let loop ([func f]
    ...
    (if (> start end)
        (reverse result)
        (loop func (+ start step) end inc (cons (func start) result)))
    ))
...
(define (num-concat x y)
  (let* ([str1 (number->string x)]
         [str2 (number->string y)]
         [+ (λ (x y) (string->number (string-append str1 str2)))])
...
(define (compute-perimeter points)
  (let loop ([L points]
             [curr (car points)]
             [next (cadr points)]
             [dist (distance (car points) (cadr points))])
      (if (= 2 (length L))
         (+ dist (distance next (car points)))
          (loop (cdr L) next (caddr L) (+ dist (distance curr next)))
          )))
...
(define (sumList L res)
  (if (null? L)
      res
      (sumList (cdr L) (+ res (car L)))))

(define (3-sequence-max numbers separator)
  (let*-values (
                ((seq1 seq) (splitf-at numbers(λ (x) (not (= x separator)))))
                ((seq2 seq3) (splitf-at (cdr seq) (λ (x) (not (= x separator)))))
                )
    (let* ([s1 (sumList seq1 0)]
           [s2 (sumList seq2 0)]
           [s3 (sumList (cdr seq3) 0)])
      (if (> s1 s2)
          (if (> s1 s3)
              s1
              s3)
...
(define (find-all-suffixes number)
  (let suf ([L (number->list number)]
              [result '()])
    (if (null? L)
        (reverse result)
        (suf (cdr L) (cons (list-num-concat L) result)))
    ))
...
(define (run initial-state final-state next)
  (let loop ([initial initial-state]
             [result '()])
             (if (= initial final-state)
                 (reverse (cons initial result))
                 (loop (next initial) (cons initial result)))
              ))

...
public class Main {
	static class Task {
		...
		private ArrayList<Integer> getResult() {
			...
			ArrayList<Integer> selectat = new ArrayList<>();
			for (int i = 0; i <= n; i++)  {
				d.add(100999);
				P.add(0);
				selectat.add(0);
			...
			while (!pq.isEmpty()) {
				Edge u = pq.poll();
				if (selectat.get(u.node) == 0) {
					selectat.set(u.node, 1);
					for (Edge edge : adj[u.node]) {
						if (selectat.get(edge.node) == 0 && d.get(edge.node) > d.get(u.node) + edge.cost) {
							d.set(edge.node, d.get(u.node) + edge.cost);
							P.set(edge.node, u.node);
							pq.add(new Edge(edge.node, d.get(edge.node)));
						}
					}
				}
			}

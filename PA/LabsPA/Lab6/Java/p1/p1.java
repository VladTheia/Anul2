...
//
//        if (heaps[0] == 2 || heaps[0] == 3 || heaps[0] == 4 && (heaps[1] + heaps[2] == 0))
//            return Inf;
//        if (heaps[1] == 2 || heaps[1] == 3 || heaps[1] == 4 && (heaps[0] + heaps[2] == 0))
//            return Inf;
//        if (heaps[2] == 2 || heaps[2] == 3 || heaps[2] == 4 && (heaps[0] + heaps[1] == 0))
//            return Inf;
...
class p1 {
    ...
    public static Pair<Integer, Move> minimax(Nim init, int player, int depth) {
        ...
        Move bestMove = moves.get(randomInt);

        for (Move move : moves) {
            Nim clone = (Nim) init.clone();
            clone.apply_move(move);

            int score = -minimax(clone , (-1) * player, depth - 1).first;

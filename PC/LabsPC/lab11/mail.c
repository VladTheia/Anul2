...
void handle_smtp() {
    ...
    n = inet_aton(SERVER_IP, &servaddr.sin_addr);
    DIE(n < 0, "inet_aton");    
    ...
    n = read(sockfd, recvbuf, MAXLEN -1);
    DIE(n < 0, "read");
    printf("Am primit: %s\n", recvbuf);
    ...
    sprintf(sendbuf, "MAIL FROM: <vlad.theia.madalin@gmail.com>");
    send_command(sockfd, sendbuf, "250");

    // TODO: trimis destinatie mail si citit raspuns
    sprintf(sendbuf, "RCPT TO: <vlad.theia.madalin@gmail.com>");
    send_command(sockfd, sendbuf, "250");
    ...
    sprintf(sendbuf, "Subject: Theia" NEWLINE
					"From: Radu Ciobanu <radu@radu.com>" NEWLINE
					"To: Mihai Popescu <mihai@mihai.com>" NEWLINE
					"Mime-Version: 1.0" NEWLINE
					"Content-Type: multipart/mixed; boundary=rrr" NEWLINE NEWLINE
					"--rrr" NEWLINE
					"Content-Type: text/plain" NEWLINE NEWLINE
					"Hello, this is my message" NEWLINE NEWLINE
					"--rrr" NEWLINE
